...
import { checkAndIssueMilestoneDocuments, calculatePaymentProgress } from '../utils/milestoneService.js';
...
export const createPaymentSchedule = async (req, res) => {
  try {
    ...
    if (plot.status !== 'reserved') {
      return res.status(400).json({ success: false, message: 'Plot must be reserved before creating payment schedule' });
    }

    const existingSchedules = await PaymentSchedule.find({ plotId });
    if (existingSchedules.length > 0) {
      return res.status(400).json({ success: false, message: 'Payment schedule already exists for this plot' });
    }
...
export const getPaymentSchedule = async (req, res) => {
  try {
    ...
    const schedules = await PaymentSchedule.find({ plotId }).sort({ installmentNumber: 1 });
    
    const schedulesWithInstallments = await Promise.all(
      schedules.map(async (schedule) => {
        const installments = await PaymentInstallment.find({ paymentScheduleId: schedule._id });
...
export const makePayment = async (req, res) => {
  try {
    ...
    const schedule = await PaymentSchedule.findById(installment.paymentScheduleId);
    if (!schedule) {
    ...
    if (plot.purchaserId?.toString() !== req.user.purchaserId?.toString()) {
      return res.status(403).json({ success: false, message: 'Not authorized to make payment for this plot' });
    }
    ...
    const milestoneResult = await checkAndIssueMilestoneDocuments(schedule.plotId);
...
export const getMyPayments = async (req, res) => {
  try {
    ...
    if (!purchaserId) {
      return res.status(403).json({ success: false, message: 'Only purchasers can view their payments' });
    }

    const plots = await Plot.find({ purchaserId });
    const plotIds = plots.map(p => p._id);

    const schedules = await PaymentSchedule.find({ plotId: { $in: plotIds } })
  ...
  try {
    ...
    for (const inst of overdueInstallments) {
      ...
      if (inst.paymentScheduleId) {
        const schedule = await PaymentSchedule.findById(inst.paymentScheduleId._id);
