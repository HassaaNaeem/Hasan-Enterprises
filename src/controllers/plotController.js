...
export const createPlot = async (req, res) => {
  try {
    ...
    if (existingPlot) {
      return res.status(400).json({ success: false, message: 'Plot number already exists' });
    }
...
export const applyForPlot = async (req, res) => {
  try {
    ...
    if (!purchaserId) {
      return res.status(403).json({ success: false, message: 'Only purchasers can apply for plots' });
    }
...
export const uploadPlotDocuments = async (req, res) => {
  try {
    ...
    if (!plotDetails) {
      return res.status(404).json({ success: false, message: 'Plot details not found' });
    }
    ...
    const allDocsUploaded = plotDetails.plotMapUri && 
...
export const getMyPlots = async (req, res) => {
  try {
    ...
    if (!purchaserId) {
      return res.status(403).json({ success: false, message: 'Only purchasers can view their plots' });
    }
